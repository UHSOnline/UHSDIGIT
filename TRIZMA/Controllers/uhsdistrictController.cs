...
{
    public class uhsdistrictController : Controller
    {
        ...
        public ActionResult District(int projectID, int taskOrderID, int Int1)
        {
        ...
        public ActionResult _bck1010008()
        {
                                                                                ...
                                                                                && returnTaskOrdersIDlist.Contains(53)))
            {
                ...
                if (userTypeid == 2)
                {
                    List<int> list1 = new List<int>(opa.DISTRICTSDbs.Where(s => s.compid == 1).Select(s => s.ID).Distinct());
                    var itemdt = opb.UHSOMD11vDbs.Where(s => s.dctpid == 1010008 && s.tspdid == 1 && list1.Contains(s.distid) && s.yyyymmdd >= l2week)
                    ...
                    return PartialView("_bck1010008", data.ToList());

                }
                else if (usertitle == 1)
                {
                    int pre1 = db.agentsDbs.Where(s => s.ID == userid).Select(s => s.divid).First();
                    List<int> pro1 = new List<int>(opa.DISTRICTSDbs.Where(s => s.divID == pre1 && s.compid == 1).Select(s => s.ID).Distinct().ToList());                    

                    var itemdt = opb.UHSOMD11vDbs.Where(s => s.dctpid == 1010008 && s.tspdid == 1 && pro1.Contains(s.distid) && s.yyyymmdd >= l2week)
                                                .Select(s => new { ID = s.ID, crdate = s.crdate, yyyymm = s.yyyymm, weeknr = s.weeknr, dctpid = s.dctpid, distid = s.distid, distnm = s.distnm });
                    List<UHSOMD11vDb> data = new List<UHSOMD11vDb>();
                    foreach (var item in itemdt)
                    {
                ...
                {
                    var list1 = opb.UHSUSAC1DISTRICTDbs.Where(s => s.MGRID == userid).Select(s => s.DISTID).Distinct().ToList();
            ...
            return PartialView("_bck1010008");
        }
        public ActionResult getEditPar1010008(int dctp, int id1, int id2, string id3)
        {
            ...
            int cnt1 = opb.UHSUSAC1DISTRICTDbs.Where(s => s.MGRID == userid && s.DISTID == id1).Count();
                                                                                ...
                                                                                && cnt1 >= 1))
            {
                ...
                var dtus01 = opa.DATAOPATIME_FRAMEDbs.Where(s => s.dateYMD == dti).Select(s => s.dateus).First();
                ...
                if (id2 == 1)
                {
                    var wchk1 = opa.DISTRICTSDbs.Where(s => s.ID == id1 && s.compid == 1).Select(s => s.wchk).First();
                    ...
                    List<UHSOMD11vDb> UHSOMD11vDbA = opb.UHSOMD11vDbs.Where(s => s.dctpid == 1010008 && s.tspdid == 1 && s.distid == id1 && s.crdate == dts).ToList();
                    var a11 = opb.UHSOMD11vDbs.Where(s => s.dctpid == 1010008 && s.tspdid == 1 && s.distid == id1 && s.crdate == dts).Select(s => s.ID).First();
                    ...
                    List<UHSOMD11vDb> UHSOMD11vDbB = opb.UHSOMD11vDbs.Where(s => s.dctpid == 1010008 && s.tspdid == 2 && s.distid == id1 && s.yyyymm == per1 && s.weeknr == wek1).ToList();
                    var a12 = opb.UHSOMD11vDbs.Where(s => s.dctpid == 1010008 && s.tspdid == 2 && s.distid == id1 && s.yyyymm == per1 && s.weeknr == wek1).Select(s => s.ID).First();
                    ...
                    List<UHSOMD11vDb> UHSOMD11vDbC = opb.UHSOMD11vDbs.Where(s => s.dctpid == 1010008 && s.tspdid == 3 && s.distid == id1 && s.yyyymm == per2).ToList();
                    var a13 = opb.UHSOMD11vDbs.Where(s => s.dctpid == 1010008 && s.tspdid == 3 && s.distid == id1 && s.yyyymm == per2).Select(s => s.ID).First();
                    ...
                    List<UHSOMD11vDb> UHSOMD11vDbD = opb.UHSOMD11vDbs.Where(s => s.dctpid == 1010008 && s.tspdid == 4 && s.distid == id1 && qtrdates.Contains(s.crdate)).ToList();
                    var a14 = opb.UHSOMD11vDbs.Where(s => s.dctpid == 1010008 && s.tspdid == 4 && s.distid == id1 && qtrdates.Contains(s.crdate)).Select(s => s.ID).First();
                    ...
                    var id330 = UHSOMD13Db.Select(s => s.smid130).First();
                    var id331 = UHSOMD13Db.Select(s => s.smid131).First();
                    var id332 = UHSOMD13Db.Select(s => s.smid132).First();
                    var id333 = UHSOMD13Db.Select(s => s.smid133).First();
                    var id334 = UHSOMD13Db.Select(s => s.smid134).First();
                    var id335 = UHSOMD13Db.Select(s => s.smid135).First();
                    ...
                    List1.CZsmid130 = db.agentsDbs.Where(s => s.ID == id330).Select(s => s.agentName).First();
                    List1.CZsmid131 = db.agentsDbs.Where(s => s.ID == id331).Select(s => s.agentName).First();
                    List1.CZsmid132 = db.agentsDbs.Where(s => s.ID == id332).Select(s => s.agentName).First();
                    List1.CZsmid133 = db.agentsDbs.Where(s => s.ID == id333).Select(s => s.agentName).First();
                    List1.CZsmid134 = db.agentsDbs.Where(s => s.ID == id334).Select(s => s.agentName).First();
                    List1.CZsmid135 = db.agentsDbs.Where(s => s.ID == id335).Select(s => s.agentName).First();
                    ...
                    List1.CZcmdt130 = UHSOMD13Db.Select(s => s.cmdt130).First();
                    List1.CZcmdt131 = UHSOMD13Db.Select(s => s.cmdt131).First();
                    List1.CZcmdt132 = UHSOMD13Db.Select(s => s.cmdt132).First();
                    List1.CZcmdt133 = UHSOMD13Db.Select(s => s.cmdt133).First();
                    List1.CZcmdt134 = UHSOMD13Db.Select(s => s.cmdt134).First();
                    List1.CZcmdt135 = UHSOMD13Db.Select(s => s.cmdt135).First();
                    ...
                    List1.zacc = opa.DISTRICTSDbs.Where(s => s.compid == 1 && s.ID == id1).Select(s => s.district).First();
                    List1.zdiv = opa.DISTRICTSDbs.Where(s => s.compid == 1 && s.ID == id1).Select(s => s.divisionName).First();
                    List1.zomh = opa.DISTRICTSDbs.Where(s => s.compid == 1 && s.ID == id1).Select(s => s.OMD).First();
                    List1.zdod = opa.DISTRICTSDbs.Where(s => s.compid == 1 && s.ID == id1).Select(s => s.DOD).First();
                    ...
                    return View("getEditPar1010008", pageModel);
                }
                else
                if (id2 == 2)
                {
                    ...
                    List<UHSOMD11vDb> UHSOMD11vDbA = opb.UHSOMD11vDbs.Where(s => s.dctpid == 1010008 && s.tspdid == 1 && s.distid == id1 && s.crdate == datr01).ToList();
                    ...
                    List<UHSOMD11vDb> UHSOMD11vDbB = opb.UHSOMD11vDbs.Where(s => s.dctpid == 1010008 && s.tspdid == 2 && s.distid == id1 && s.yyyy == year01 && s.weeknr == week01).ToList();
                    var a12 = opb.UHSOMD11vDbs.Where(s => s.dctpid == 1010008 && s.tspdid == 2 && s.distid == id1 && s.yyyy == year01 && s.weeknr == week01).Select(s => s.ID).First();
                    List<UHSOMD12Db> UHSOMD12Db = opa.UHSOMD12Dbs.Where(s => s.ID == a12).ToList();
                    ...
                    List<UHSOMD11vDb> UHSOMD11vDbC = opb.UHSOMD11vDbs.Where(s => s.dctpid == 1010008 && s.tspdid == 3 && s.distid == id1 && s.yyyymm == mnth02).ToList();
                    var a13 = opb.UHSOMD11vDbs.Where(s => s.dctpid == 1010008 && s.tspdid == 3 && s.distid == id1 && s.yyyymm == mnth02).Select(s => s.ID).First();
                    List<UHSOMD13Db> UHSOMD13Db = opa.UHSOMD13Dbs.Where(s => s.ID == a13).ToList();
        ...
        public ActionResult _getDistrictdata(int IDa, int IDb)
        {
            string CurrentLoginID = User.Identity.GetUserId().ToString();
            ...
            if (IDa == 0 && IDb == 0)
            {
                if (userTypeid == 2)
                {
                    var datatable = opb.UHSWEBAABvDbs.ToList();
                    return PartialView("_getDistrictdata", datatable);
                }
                else if (usertitle == 1)
                {
                    int pre1 = db.agentsDbs.Where(s => s.ID == userid).Select(s => s.divid).First();
                    int pre2 = opa.DISTRICTSDbs.Where(s => s.divID == pre1 && s.compid == 1).Select(s => s.divID).First();
                    List<int> pro1 = new List<int>(opa.DISTRICTSDbs.Where(s => s.divID == pre2 && s.compid == 1).Select(s => s.ID).Distinct().ToList());

                    //var list1 = db.agentsDbs.Where(s => s.DODID == userid && s.ID != userid).Select(s => s.distid).Distinct().ToList();
                    var datatable = opb.UHSWEBAABvDbs.Where(s => pro1.Contains(s.ID)).ToList();
                    return PartialView("_getDistrictdata", datatable);
                }
                else if (usertitle == 2 || usertitle == 3)
                {
                    var list1 = opb.UHSUSAC1DISTRICTDbs.Where(s => s.MGRID == userid).Select(s => s.DISTID).Distinct().ToList();
                    var datatable = opb.UHSWEBAABvDbs.Where(s => list1.Contains(s.ID)).ToList();
                    return PartialView("_getDistrictdata", datatable);
                }
                else
                {
            ...
            else if (IDa != 0 && IDb == 0)
            {
                if (userTypeid == 2)
                {
                    var datatable = opb.UHSWEBAABvDbs.Where(s => s.divID == IDa).ToList();
                    return PartialView("_getDistrictdata", datatable);
                }
                else if (usertitle == 1)
                {
                    int pre1 = db.agentsDbs.Where(s => s.ID == userid).Select(s => s.distid).First();
                    int pre2 = opa.DISTRICTSDbs.Where(s => s.ID == pre1 && s.compid == 1).Select(s => s.divID).First();
                    List<int> pro1 = new List<int>(opa.DISTRICTSDbs.Where(s => s.divID == pre2 && s.compid == 1).Select(s => s.ID).Distinct().ToList());
                    //var list1 = db.agentsDbs.Where(s => s.DODID == userid && s.ID != userid).Select(s => s.distid).Distinct().ToList();
                    var datatable = opb.UHSWEBAABvDbs.Where(s => s.divID == IDa && pro1.Contains(s.ID)).ToList();
                    return PartialView("_getDistrictdata", datatable);
                }
                else if (usertitle == 2 || usertitle == 3)
                {
                    var list1 = opb.UHSUSAC1DISTRICTDbs.Where(s => s.MGRID == userid).Select(s => s.DISTID).Distinct().ToList();
                    var datatable = opb.UHSWEBAABvDbs.Where(s => s.divID == IDa && list1.Contains(s.ID)).ToList();
                    return PartialView("_getDistrictdata", datatable);
                }
                else
                {
            ...
            else if (IDb != 0)
            {
                if (userTypeid == 2)
                {
                    var datatable = opb.UHSWEBAABvDbs.Where(s => s.ID == IDb).ToList();
                    return PartialView("_getDistrictdata", datatable);
                }
                else if (usertitle == 1)
                {
        ...
        {
            var datatable = opa.DISTRICTSDbs.Where(s => s.ID == 0 || (s.divID == ID && s.compid == 1)).OrderBy(s => s.ID).Select(s => new { ID = s.ID, district = s.district }).ToList();
            return Json(datatable, JsonRequestBehavior.AllowGet);
        }
