...
{
    public class uhsuserDistrictController : Controller
    {
        ...
        public ActionResult Index(int projectID, int taskOrderID, int Int1)
        {
            ...
            {
                ViewBag.dist1 = opb.UHSUSAC1DISTRICTDbs.OrderBy(s => s.agentName).Select(s => new { DISTID = s.DISTID, MGRID = s.MGRID, agentName = s.agentName }).ToList();
                ViewBag.btnSel = 3;
                return View();
            }
            else
        ...
        {
                var list1 = opb.UHSUSAC1DISTRICTDbs.Where(s => s.MGRID == ID).Select(s => s.DISTID).Distinct().ToList();
                var datatable = opa.DISTRICTSDbs.Where(c => c.compid == 1 && !list1.Contains(c.ID))
                                               .OrderBy(c => c.ID)
        ...
        public ActionResult EditDoc(string id)
        {
            ...
            if (userTypeInt == 2 && CurrentLoginID == User.Identity.GetUserId().ToString() && returnProjectIDlist.Contains(6)
                                                                                            && returnTaskOrdersIDlist.Contains(61))
            {
                var IDD = opa.UHSUSAT1DISTRICTDbs.Where(s => s.ID == id).Select(s => s.MGRID).First();
                var acc1 = opa.UHSUSAT1DISTRICTDbs.Where(s => s.ID == id).Select(s => s.DISTID).First();
                var list1 = opb.UHSUSAC1DISTRICTDbs.Where(s => s.MGRID == IDD).Select(s => s.DISTID).Distinct().ToList();
                var datatable = opa.DISTRICTSDbs.Where(c => c.compid == 1 && (c.ID == acc1 || !list1.Contains(c.ID)))
                                               .OrderBy(c => c.ID)
                                               .Select(c => new { ID = c.ID, Value = c.district });
                

                ViewBag.users1 = new SelectList(dbv.agentsViewDbs.OrderBy(s => s.agentName), "ID", "agentNameID2");
                ViewBag.dist1 = new SelectList(opa.DISTRICTSDbs.Where(c => c.compid == 1 && (c.ID == acc1 || !list1.Contains(c.ID))).OrderBy(c => c.ID).Select(c => new { ID = c.ID, district = c.district }), "ID", "district");
                ViewBag.userid = userIDselectInt;
                ViewBag.data1 = opa.UHSUSAT1DISTRICTDbs.Where(s => s.ID == id);
        ...
        {
            var data = from s in opb.UHSUSAC1DISTRICTDbs select s;
