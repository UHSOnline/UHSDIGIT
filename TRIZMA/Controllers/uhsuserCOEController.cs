...
{
    public class uhsuserCOEController : Controller
    {
        ...
        public ActionResult Index(int projectID, int taskOrderID, int Int1)
        {
            ...
            {
                ViewBag.dist1 = opb.UHSUSAC1COEDbs.OrderBy(s => s.agentName).Select(s => new { DISTID = s.DISTID, MGRID = s.MGRID, agentName = s.agentName }).ToList();
                ViewBag.btnSel = 5;
                return View();
            }
            else
        ...
        {
                var list1 = opb.UHSUSAC1COEDbs.Where(s => s.MGRID == ID).Select(s => s.DISTID).Distinct().ToList();
                var datatable = opa.DISTCOEDbs.Where(c => c.compid == 1 && !list1.Contains(c.ID))
                                               .OrderBy(c => c.ID)
        ...
        public ActionResult CreateNew(UHSUSAT1COEDb UHSUSAT1COEDb)
        {
            string CurrentLoginID = User.Identity.GetUserId().ToString();
            ...
            if (userTypeInt == 2 && CurrentLoginID == User.Identity.GetUserId().ToString() && returnProjectIDlist.Contains(6)
                                                                                            && returnTaskOrdersIDlist.Contains(61))
            {
                if (UHSUSAT1COEDb.MGRID == 0 || UHSUSAT1COEDb.DISTID == 0)
                {
                    return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
                }
                else
                {
                        ...
                        try
                        {
                            opa.UHSUSAT1COEDbs.Add(UHSUSAT1COEDb);
                            opa.SaveChanges();
                            var conf = 1;
        ...
        public ActionResult EditDoc(string id)
        {
            ...
            if (userTypeInt == 2 && CurrentLoginID == User.Identity.GetUserId().ToString() && returnProjectIDlist.Contains(6)
                                                                                            && returnTaskOrdersIDlist.Contains(61))
            {
                var IDD = opa.UHSUSAT1COEDbs.Where(s => s.ID == id).Select(s => s.MGRID).First();
                var acc1 = opa.UHSUSAT1COEDbs.Where(s => s.ID == id).Select(s => s.DISTID).First();
                var list1 = opb.UHSUSAC1COEDbs.Where(s => s.MGRID == IDD).Select(s => s.DISTID).Distinct().ToList();
                var datatable = opa.DISTCOEDbs.Where(c => c.compid == 1 && (c.ID == acc1 || !list1.Contains(c.ID)))
                                               .OrderBy(c => c.ID)
                                               .Select(c => new { ID = c.ID, Value = c.district });
                

                ViewBag.users1 = new SelectList(dbv.agentsViewDbs.OrderBy(s => s.agentName), "ID", "agentNameID2");
                ViewBag.dist1 = new SelectList(opa.DISTCOEDbs.Where(c => c.compid == 1 && (c.ID == acc1 || !list1.Contains(c.ID))).OrderBy(c => c.ID).Select(c => new { ID = c.ID, district = c.district }), "ID", "district");
                ViewBag.userid = userIDselectInt;
                ViewBag.data1 = opa.UHSUSAT1COEDbs.Where(s => s.ID == id);
        ...
        [HttpPost]
        //[ValidateAntiForgeryToken]
        public ActionResult EditEx(UHSUSAT1COEDb model)
        {
        ...
        public ActionResult Delete(string id)
        {
                                                                                            ...
                                                                                            && returnTaskOrdersIDlist.Contains(61))
            {
                ...
                UHSUSAT1COEDb UHSUSAT1COEDb = opa.UHSUSAT1COEDbs.Find(id);
                if (UHSUSAT1COEDb == null)
                {
                    return HttpNotFound();
                }
                return View(UHSUSAT1COEDb);
        ...
        public ActionResult DeleteConfirmed(string id)
        {
            ...
            {
                UHSUSAT1COEDb UHSUSAT1COEDb = opa.UHSUSAT1COEDbs.Find(id);
                opa.UHSUSAT1COEDbs.Remove(UHSUSAT1COEDb);
                opa.SaveChanges();
                return RedirectToAction("Index", "uhsuserCOE", new { projectID = 6, taskOrderID = 61, Int1 = 1 });
            }
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
        }
        public ActionResult getCOEmap(int ID)
        {
            string CurrentLoginID = User.Identity.GetUserId().ToString();
        ...
        {
            var data = from s in opb.UHSUSAC1COEDbs select s;
